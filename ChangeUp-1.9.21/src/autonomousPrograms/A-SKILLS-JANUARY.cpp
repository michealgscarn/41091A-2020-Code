...
//   // ----- Reset Position ----- //
  ...
  int resetTimeout = pros::millis()+500;
  double maxResetX = 0;
  double maxResetTheta = 0;

  while(pros::millis()<resetTimeout){
    quickAlign();
    maxResetX=std::max(thetaVal,maxResetX);
    maxResetTheta=std::max(XYVal,maxResetTheta);
    pros::delay(20);
  }
